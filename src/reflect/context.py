...
def _briefing_from_wiki(wiki_dir, fmt):
    ...
    # Group active pages by category
    by_category = {}
    for page in pages:
    ...
    for section_def in fmt["sections"]:
        slug = slugify(section_def["name"])
        cat_pages = by_category.get(slug, [])
        if not cat_pages:
            continue

        # Apply max_bullets limit
        max_b = section_def.get("max_bullets", 8)
        # Sort by updated date (newest first)
        cat_pages.sort(key=lambda p: p.get("updated", ""), reverse=True)
        cat_pages = cat_pages[:max_b]

        sections.append(f"\n## {section_def['name']}\n")

        for page in cat_pages:
            # Build citation from sources
            cite = ""
            if page.get("sources"):
                src = page["sources"][0]
            ...
            summary = page.get("summary") or page["title"]
            sections.append(f"- **{page['title']}** — {summary}{cite}")

    # If only the heading was produced, no active pages matched
    if len(sections) <= 1:
        return None
    ...
    # Check line budget
    max_lines = fmt.get("max_lines", 150)
    lines = result.split("\n")
